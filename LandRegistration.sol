...
bytes32 public donemessage = "Preocess Done";
address public government;
modifier only_gov(){
    require(government == msg.sender);
    _;
}
event VerifyLog(bool);
event PropertyRegisterLog(bool);
event TransferOwnerLog(bool);
mapping(
    bytes32 => mapping(
       address => LandDetails 
        )
    ...
    function LandRegistration(address _governmentaddress)public{
              government = _governmentaddress;
    }
    ...
    emit PropertyRegisterLog(landdetails[_propertyID][_accountID].Registered_status);
    ...
    only_gov(){
        ...
        emit VerifyLog(landdetails[_propertyID][_accountID].Activestatus);
    }
    ...
    emit TransferOwnerLog(landdetails[_propertyID][trto_accountid].Activestatus);
    }    
